...
class CustomView {
	...
	protected function removeMount($mount, $path) {
		if ($mount instanceof MoveableMount) {
			...
			$this->changeLockCustom($relPath, ILockingProvider::LOCK_EXCLUSIVE, true);
			$result = $mount->removeMount();
			$this->changeLockCustom($relPath, ILockingProvider::LOCK_SHARED, true);
			if ($result) {
				\OC_Hook::emit(
	...
	public function file_put_contents($path, $data) {
		if (is_resource($data)) { //not having to deal with streams in file_put_contents makes life easier
			...
			if (Filesystem::isValidPath($path)
				...
				$this->changeLockCustom($path, ILockingProvider::LOCK_EXCLUSIVE);
				...
				if ($target) {
					...
					$this->changeLockCustom($path, ILockingProvider::LOCK_SHARED);
	...
	public function rename($path1, $path2) {
		...
		if (
			...
			if ($run) {
				...
				$this->changeLockCustom($path1, ILockingProvider::LOCK_EXCLUSIVE, true);
				$this->changeLockCustom($path2, ILockingProvider::LOCK_EXCLUSIVE, true);
				...
				$this->changeLockCustom($path1, ILockingProvider::LOCK_SHARED, true);
				$this->changeLockCustom($path2, ILockingProvider::LOCK_SHARED, true);
		...
		if (
			...
			try {
				...
				if ($run) {
					...
					$this->changeLockCustom($path2, ILockingProvider::LOCK_EXCLUSIVE);
					...
					$this->changeLockCustom($path2, ILockingProvider::LOCK_SHARED);
					$lockTypePath2 = ILockingProvider::LOCK_SHARED;
	 ...
	 * @throws \OCP\Lock\LockedException if the path is already locked
	 */
	public function changeLockCustom($path, $type, $lockMountPoint = false) {
		$path = Filesystem::normalizePath($path);
		$absolutePath = $this->getAbsolutePath($path);
		$absolutePath = Filesystem::normalizePath($absolutePath);
		if (!$this->shouldLockFile($absolutePath)) {
	 ...
	 * @return bool
	 */
	public function moveFromStorageCustom(\OCP\Files\Storage $sourceStorage, $sourceInternalPath, $targetInternalPath, $preserveMtime = true, $isRename = false) {
